...
Bij elke aanvraag wordt automatisch een offerte als PDF, Word en Excel gegenereerd en als
bijlage meegestuurd naar info@pharmaclean.nl.
...
- `api/quote-docs.js`: bouwt de offerte als PDF (pdfkit), Word (docx) en Excel (exceljs)
  op basis van de itemlijst die de frontend meestuurt
- `package.json`: bevat de dependencies (`resend`, `pdfkit`, `docx`, `exceljs`)
...
Vercel installeert `pdfkit`, `docx` en `exceljs` automatisch bij het builden, dat vergt geen
extra configuratie.
